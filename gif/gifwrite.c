...
ColorMapObject *ColorMapObject_val( value cmap )
{
  ...
  len = list_length(cmap);
fprintf(stderr, "Creating map with length = %d ...\n", len);
fflush(stderr);
  cmapobj = MakeMapObject( len, NULL );
  for(i=0, l= cmap; i< len; i++, l = Field(l,1)){
    cmapobj->Colors[i].Red   = Int_val(Field(Field(l,0),0));
    cmapobj->Colors[i].Green = Int_val(Field(Field(l,0),1));
    cmapobj->Colors[i].Blue  = Int_val(Field(Field(l,0),2));
  }
  return cmapobj; 
}
*/
...
#define nodef(f) value f(){failwith( "gif is not supported" ); \
  return(Val_unit);}

nodef(write_gif_file)
nodef(eGifOpenFileName)
nodef(eGifCloseFile)
nodef(eGifPutScreenDesc)
nodef(eGifPutImageDesc)
nodef(eGifPutLine)
nodef(eGifPutExtension)
