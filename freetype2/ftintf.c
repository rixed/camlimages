...
value get_Kerning(value face, value left_glyph, value right_glyph, value kern_mode) {
  CAMLparam4(face, left_glyph, right_glyph, kern_mode);
  CAMLlocal1(res);

  FT_Vector akerning;
  int err = FT_Get_Kerning( *(FT_Face *)face, 
		  Int_val(left_glyph), Int_val(right_glyph),
		  Int_val(kern_mode), &akerning );

  res = alloc_tuple(2);
  Store_field(res, 0, Val_int(akerning.x) );
  Store_field(res, 1, Val_int(akerning.y) );
